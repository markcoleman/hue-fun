...
function extractHueErrorDetails(payload: unknown): HueErrorDetail[] {
  if (!payload) {
    return [];
  }
...
function normalizeFailure(result: GeneratedFieldsResult<unknown>): never {
  if (result.response) {
...
function unwrapResult<T>(result: GeneratedFieldsResult<T>, operation: string): T {
...
function unwrapSingleResource<T extends { id: string }>(items: T[], id: string, operation: string): T {
